...
static USB_HID_CTL_STA_t IAP_Transport_Send_ACK(uint8_t err){
    ...
    return bsp_usb_hid_ctl_send((uint8_t *)&ACK, sizeof(IAP_Transport_ACK_t));
}
...
static uint8_t IAP_Transport_send_single_pack(uint8_t first_flag, uint16_t pack_num, uint8_t *payload_data, uint16_t payload_len){
    ...
    // send data.
    return (uint8_t)bsp_usb_hid_ctl_send(sendCtl.buf, sendCtl.size);
}
...
// send ok
static void bsp_usb_hid_ctl_send_complete_callback(void){
...
// recv callback.
static void bsp_usb_hid_ctl_recv_callback(uint8_t *data, uint16_t len){
    ...
    // platform_printf("SEND(%d) ...\n", bsp_usb_hid_ctl_send((uint8_t *)"\x88\x99\x33\x44", 4) );
